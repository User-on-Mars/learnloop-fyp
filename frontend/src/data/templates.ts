...
export interface SkillMapTemplate {
  ...
  description: string;   // 0-120 chars
  icon: string;          // valid icon from iconLibrary
  goal: string;          // 1-16 chars
  nodes: NodeDefinition[];
}

/**
 * Given a desired title and a set of existing titles,
 * returns a unique title by appending a numeric suffix if needed.
 */
export function deduplicateTitle(title: string, existingTitles: Set<string> | string[]): string {
  const existing = existingTitles instanceof Set ? existingTitles : new Set(existingTitles);
  if (!existing.has(title)) return title;
  let suffix = 2;
  while (existing.has(`${title} (${suffix})`)) {
    suffix++;
  }
  return `${title} (${suffix})`;
}

export const TEMPLATES: SkillMapTemplate[] = [
  {
