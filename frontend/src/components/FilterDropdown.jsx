...
export default function FilterDropdown({ value, onChange, options, minWidth = 160 }) {
  ...
  return (
    <div ref={ref} className="relative inline-block" style={{ minWidth }}>
      ...
      {open && (
        <div className="absolute left-0 z-[100] mt-1.5 min-w-full bg-white border border-[#d4dbc9] rounded-xl shadow-lg py-1 max-h-60 overflow-y-auto">
          {options.map(opt => (
            <button key={opt.value} type="button"
              onClick={() => { onChange(opt.value); setOpen(false); }}
              className={`w-full text-left px-4 py-2 text-[13px] transition-colors whitespace-nowrap ${
                opt.value === value
