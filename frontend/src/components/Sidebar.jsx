      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-[#dde1d6] flex items-center px-4 gap-4 z-40 shadow-sm shadow-black/[0.02]">
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => navigate("/skills")} title="Create Skill Map"
            className="w-9 h-9 rounded-lg flex items-center justify-center text-[#565c52] hover:bg-[#eef0ea] transition-colors">
      ...
      <aside className={`fixed top-16 left-0 bottom-0 bg-white border-r border-[#dde1d6] flex-col items-center py-3 z-30 transition-all duration-200 overflow-hidden hidden md:flex shadow-sm shadow-black/[0.02] ${sidebarOpen ? 'w-[56px]' : 'w-0 border-r-0'}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => navigate(item.path)} title={item.label}
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${active ? 'bg-[#2e5023] text-white shadow-sm' : 'text-[#8a9180] hover:bg-[#eef0ea] hover:text-[#3a4a30]'}`}>
                <Icon />
              </button>
            );
          })}
          {isAdmin && (
            <>
              ...
              <button onClick={handleAdminClick} title="Admin Panel"
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${location.pathname.startsWith('/admin') ? 'bg-red-500 text-white shadow-sm' : 'text-[#8a9180] hover:bg-[#eef0ea] hover:text-[#3a4a30]'}`}>
        ...
        {isFree && (
          <button onClick={() => navigate('/subscription')} title="Upgrade to Pro"
            className="w-9 h-9 rounded-lg flex items-center justify-center text-amber-500 hover:bg-amber-50 transition-colors mb-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3l3.057-3L12 3.5 15.943 0 19 3l-2 7H7L5 3zM7 10h10l1 10H6l1-10z" /></svg>
          </button>
        )}
      </aside>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 border border-[#e2e6dc]">
            <div className="flex flex-col items-center text-center">
              {hasActiveSessions ? (
                <>
                  <AlertIcon />
                  <h3 className="text-lg font-bold text-site-ink mt-4 mb-2">Active Sessions Running</h3>
                  <p className="text-site-muted text-sm mb-4">
                    You have {activeSessions.length} active session{activeSessions.length > 1 ? 's' : ''}
                ...
                <>
                  ...
                  <h3 className="text-lg font-bold text-site-ink mt-4 mb-2">Sign out?</h3>
                  <p className="text-site-muted text-sm mb-4">Are you sure you want to sign out?</p>
                </>
              )}
              <div className="flex gap-3 w-full">
                <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 py-2.5 border border-[#e2e6dc] text-site-muted rounded-lg font-medium hover:bg-[#f5f7f2] transition-colors">No</button>
                <button onClick={performLogout} className="flex-1 py-2.5 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors">{hasActiveSessions ? 'Sign Out Anyway' : 'Yes'}</button>
              </div>
            </div>
          </div>
        </div>
